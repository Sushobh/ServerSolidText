...
internal interface ETPostRepository : CrudRepository<ETPost,BigInteger>{
    ...
    @Query("select id,post.by_user_id as byUser,post.time,post_text as postText,post.status,time as addedtime from post where id = ?1", nativeQuery = true)
    fun getPostById(id : BigInteger) : PJPostFeedItem?
